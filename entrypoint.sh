...
if [ "$db_type_lc" = "mysql" ] || [ "$db_type_lc" = "postgres" ] || [ "$db_type_lc" = "postgresql" ]; then
    if [ -z "${DB_DSN:-}" ]; then
        if [ -z "${DB_HOST:-}" ] || [ -z "${DB_USER:-}" ] || [ -z "${DB_NAME:-}" ]; then
            ...
            echo "[DB] type=${DB_TYPE} host=${DB_HOST} port=${DB_PORT:-} name=${DB_NAME} user=${DB_USER}"
        fi
    else
        echo "[DB] type=${DB_TYPE} using DB_DSN"
    fi
...
exec ./openlist server --no-prefix --data "$OPENLIST_DATA_DIR"
